(defmodule ENV (import MAIN ?ALL) (export deftemplate k-cell k-per-row k-per-col)) ; importa tutto quello che il main ha da importare, ma esporta soltanto alcuni template (quelli che rappresentano la conoscenza iniziale dell'agente)

;  ________________ TEMPLATES ________________

(deftemplate cell ; ogni cella ha le sue coordinate, contenuto e stato
	(slot x)
	(slot y)
...
(deftemplate boat-hor ; nave orizzontale
	...
	(multislot ys)
	(slot size)
	(multislot status (allowed-values safe hit)) ; quante delle celle della nave sono state colpite o sono al sicuro
)
...
(deftemplate k-cell ; rappresentazione della cella data all'inizio del gioco
	(slot x)
	(slot y)
	(slot content (allowed-values water left right middle top bot sub))
)

(deftemplate k-per-row ; per ogni riga il numero di celle occupate da una porzione di nave
	(slot row)
	(slot num)
)

(deftemplate k-per-col ; per ogni colonna il numero di celle occupate da una porzione di nave
	(slot col)
	(slot num)
)
